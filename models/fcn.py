...
class FCN8s(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.abs(score)  # size=(N, n_class, x.H/1, x.W/1)
...
def fcns():
    ...
    with torch.no_grad():
        conv_weight = vgg_model._modules['features'][0].weight.clone()
        layer = nn.Conv2d(4, 64, kernel_size=3, stride=1, padding=1)
        layer.weight[:, :3] = conv_weight
        layer.weight[:, 3] = conv_weight[:, 0]
        vgg_model._modules['features'][0] = layer
...
if __name__ == "__main__":
    ...
    input = torch.autograd.Variable(torch.randn(batch_size, 4, h, w))
